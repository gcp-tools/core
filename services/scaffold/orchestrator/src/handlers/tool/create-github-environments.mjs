...
export async function createGitHubEnvironments(input) {
    ...
    for (const env of environments) {
        try {
            ...
            const { stdout } = await execAsync(`gh api repos/${repoName}/environments --jq '.environments[].name'`);
            const existing = stdout
                .split('\n')
                .map((s) => s.trim())
                .filter(Boolean);
            if (existing.includes(env)) {
                console.error(`[info] Environment '${env}' already exists in ${repoName}. Skipping.`);
            ...
            console.error(`[error] Failed to create environment '${env}' in ${repoName}:`, error);
...
//# sourceMappingURL=create-github-environments.mjs.map