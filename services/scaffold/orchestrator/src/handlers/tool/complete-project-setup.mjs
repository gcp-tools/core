...
export async function completeProjectSetup(input) {
    ...
    try {
        ...
        if (Array.isArray(prereqResult.summary) &&
        ...
        console.error('[DEBUG] createGitHubRepo input:', JSON.stringify(createGitHubRepoInput, null, 2));
        const repoResult = await createGitHubRepo(createGitHubRepoInput);
        console.error('[DEBUG] createGitHubRepo result:', JSON.stringify(repoResult, null, 2));
        ...
        async function waitForWorkloadIdentityProviderReady({ projectId, poolId, providerId, timeoutMs = 120000, pollIntervalMs = 5000, }) {
            const start = Date.now();
            let attempt = 0;
            while (Date.now() - start < timeoutMs) {
                attempt++;
                try {
                    console.error(`[wait] Checking for WIP provider dev (attempt ${attempt})...`);
                    const { stdout } = await execAsync(`gcloud iam workload-identity-pools providers describe ${providerId} --project=${projectId} --workload-identity-pool=${poolId} --location=global --format=json`);
                    const provider = JSON.parse(stdout);
                    if (provider &&
                        (provider.state === undefined || provider.state === 'ACTIVE')) {
                        console.error(`[wait] WIP provider dev is ready after ${((Date.now() - start) / 1000).toFixed(1)}s.`);
                        return true;
                    }
                    console.error(`[wait] WIP provider dev found but not active (state: ${provider.state}). Retrying...`);
                }
                catch (e) {
            ...
            throw new Error(`Workload Identity Provider 'dev' not ready after ${timeoutMs / 1000}s`);
        }
        ...
        console.error('[DEBUG] runFoundationProjectHandler input:', JSON.stringify(runFoundationProjectHandlerInput, null, 2));
        const foundationResult = await runFoundationProjectHandler(runFoundationProjectHandlerInput);
        console.error('[DEBUG] runFoundationProjectHandler result:', JSON.stringify(foundationResult, null, 2));
        // Wait for dev WIP to be ready before proceeding
        if (foundationResult?.projectId &&
            ...
            const match = devProviderResource.match(/workloadIdentityPools\/(.+)\/providers\/(.+)$/);
            if (match) {
                ...
                console.error('[wait] Could not parse poolId/providerId from dev WIP resource string. Skipping wait.');
            }
        }
        else {
            console.error('[wait] No dev WIP resource found in foundationResult. Skipping wait.');
        }
        ...
        console.error('[DEBUG] createGitHubEnvironments input:', JSON.stringify(createGitHubEnvironmentsInput, null, 2));
        const envResult = await createGitHubEnvironments(createGitHubEnvironmentsInput);
        console.error('[DEBUG] createGitHubEnvironments result:', JSON.stringify(envResult, null, 2));
        ...
        console.error('[DEBUG] setupGitHubSecrets input:', JSON.stringify(setupGitHubSecretsInput, null, 2));
        const secretsResult = await setupGitHubSecrets(setupGitHubSecretsInput);
        console.error('[DEBUG] setupGitHubSecrets result:', JSON.stringify(secretsResult, null, 2));
        ...
        console.error('[DEBUG] createSkeletonApp input:', JSON.stringify(createSkeletonAppInput, null, 2));
        const skeletonResult = await createSkeletonApp(createSkeletonAppInput);
        console.error('[DEBUG] createSkeletonApp result:', JSON.stringify(skeletonResult, null, 2));
        results.step5 = {
...
//# sourceMappingURL=complete-project-setup.mjs.map