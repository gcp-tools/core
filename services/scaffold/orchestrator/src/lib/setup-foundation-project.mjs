...
const InputSchema = z.object({
    ...
    regions: z
        ...
        .split(',')
        .map((r) => r.trim())
        .filter(Boolean).length > 0, 'At least one region must be specified'),
...
async function isBillingLinked(projectId, billingAccount) {
    try {
...
async function serviceAccountExists(projectId, email) {
    try {
...
async function workloadIdentityPoolExists(projectId, poolId) {
    try {
        const { stdout } = await exec(`gcloud iam workload-identity-pools describe ${poolId} --project=${projectId} --location=global --format="value(name)"`);
        return stdout.trim() !== '';
    }
    catch {
        return false;
...
async function workloadIdentityProviderExists(projectId, poolId, providerId) {
    try {
        const { stdout } = await exec(`gcloud iam workload-identity-pools providers describe ${providerId} --project=${projectId} --workload-identity-pool=${poolId} --location=global --format="value(name)"`);
        return stdout.trim() !== '';
    }
    catch {
        return false;
...
export async function runFoundationProject(args) {
...
//# sourceMappingURL=setup-foundation-project.mjs.map